...
const Select=(event)=>{
  ...
  const handleFileUpload = async (file) => {
    try {
      ...
      const storageRef = ref(
        storage,
        `manageEntertainmentSection/${postId}`
      );
  ...
  try {  const SavedPostResonse = await fetch("https://18.218.162.154:8443/server/manageEntertainmentSection", {
      method: "POST",
      credentials: "include",
      headers: {
  ...
  return (
    <BoxInsider>
      ...
      <Typography
        ...
        sx={{ margin: "30px" }}
      >
        Dashboard/Add_manageEntertainmentSection
      </Typography>
