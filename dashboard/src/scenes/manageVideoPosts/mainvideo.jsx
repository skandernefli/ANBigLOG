import {
  ...
  const PostsList = () => {
    ...
    useEffect(() => {
      fetch("https://18.218.162.154:8443/server/mainvideo")
    ...
    const columns = [
      ...
      { field: "linkvideo", headerName: "linkvideo", width: 50 },
      { field: "linkpost", headerName: "linkpost", width: 50 },
    ...
    const handleDelete = async (_id) => {
      fetch(`https://18.218.162.154:8443/server/mainvideo/${_id}`, {
        method: "delete",
        credentials: "include",
        headers: {
    ...
    return (
      <BoxInsider>
        <Typography
          ...
          sx={{ margin: "30px" }}
        >
          Dashboard/managevideoposts/mainvideo
        </Typography>
        <Box className="controllersLayoutHomePageForm">
          <Box>
            <AnimatedButton
              className="buttonform"
              onClick={()=>{navigate("/addmainvideo")}}
              sx={{
