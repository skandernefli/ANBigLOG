...
const Select=(event)=>{
  ...
  const handleFileUpload = async (file) => {
    try {
      ...
      const storageRef = ref(
        storage,
        `manageSportsMainSection/${postId}`
      );
  ...
  try {  const SavedPostResonse = await fetch("https://18.218.162.154:8443/server/manageSportsMainSection", {
      method: "POST",
      credentials: "include",
      headers: {
  ...
  return (
    <BoxInsider>
      ...
      <Typography
        ...
        sx={{ margin: "30px" }}
      >
        Dashboard/Manage_Posts_Gallery/Add_manageSportsMainSection/
      </Typography>
