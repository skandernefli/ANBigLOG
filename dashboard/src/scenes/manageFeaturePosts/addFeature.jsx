...
const Select=(event)=>{
  ...
  try {  const SavedPostResonse = await fetch("https://18.218.162.154:8443/server/feature", {
      method: "POST",
      credentials: "include",
      headers: {
          ...
          <MenuItem key={category.id} value={category.name}>
            {category.name}
                  ...
                  />
                    ...
                    name="article"
                    error={Boolean(touched.article) && Boolean(errors.article)}
                    variant="filled"
                    label="article"
                    sx={{
                      gridColumn: "span 2",
