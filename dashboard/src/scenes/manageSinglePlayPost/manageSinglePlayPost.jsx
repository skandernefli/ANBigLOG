import {
  ...
  const PostsList = () => {
    ...
    useEffect(() => {
      fetch("https://18.218.162.154:8443/server/manageSinglePlayPost")
    ...
    const handleDelete = async (_id) => {
      fetch(`https://18.218.162.154:8443/server/manageSinglePlayPost/${_id}`, {
        method: "delete",
        credentials: "include",
        headers: {
    ...
    return (
      <BoxInsider>
        <Typography
          ...
          sx={{ margin: "30px" }}
        >
          Dashboard/Manage_Posts_Gallery/manageSinglePlayPost
        </Typography>
        <Box className="controllersLayoutHomePageForm">
          <Box>
            <AnimatedButton
              className="buttonform"
              onClick={()=>{navigate("/addmanageSinglePlayPost")}}
              sx={{
