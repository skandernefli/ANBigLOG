...
const PostsList = () => {
  ...
  useEffect(() => {
    fetch("https://18.218.162.154:8443/server/popular")
  ...
  const handleDelete = async (_id) => {
    fetch(`https://18.218.162.154:8443/server/popular/${_id}`, {
      method: "delete",
      credentials: "include",
      headers: {
  ...
  return (
    <BoxInsider>
      <Typography
        ...
        sx={{ margin: "30px" }}
      >
        Dashboard/Manage_Posts_Gallery/Popular
      </Typography>
      <Box className="controllersLayoutHomePageForm">
        <Box>
          <AnimatedButton
            className="buttonform"
            onClick={()=>{navigate("/addpopular")}}
            sx={{
