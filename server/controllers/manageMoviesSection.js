const manageMoviesSection = require('../homepagemodel/manageMoviesSection');
const getGalleryPost=(req,res)=>{
    manageMoviesSection.findById(req.params.id)
    .then(manageMoviesSection => {
      if (!manageMoviesSection) {
        return res.status(404).json({
          message: 'gallery Post not found'
        });
      }

      res.status(200).json(manageMoviesSection);
    })
    .catch(error => {
      console.log(error);
      res.status(500).json({
...
const postGalleryPost= async (req,res)=>{
    try {
        ...
        const post = await manageMoviesSection.findOneAndUpdate(
          {},
          { $push: { data: newPost } },
          { upsert: true }
        );
...
const deleteGalleryPost=(req,res)=>{
  ...
  manageMoviesSection.findOneAndUpdate(
            ...
            manageMoviesSection
              ...
              .then((manageMoviesSection) => {
                ...
                res.status(200).json({
                  ...
                  manageMoviesSection: manageMoviesSection,
                });
              })
              .catch((error) => {
                console.log(error);
