const manageSideSportsSection = require('../homepagemodel/manageSideSportsSection');
const getGalleryPost=(req,res)=>{
    manageSideSportsSection.findById(req.params.id)
    .then(manageSideSportsSection => {
      if (!manageSideSportsSection) {
        return res.status(404).json({
          message: 'gallery Post not found'
        });
      }

      res.status(200).json(manageSideSportsSection);
    })
    .catch(error => {
      console.log(error);
      res.status(500).json({
...
const postGalleryPost= async (req,res)=>{
    try {
        ...
        const post = await manageSideSportsSection.findOneAndUpdate(
          {},
          { $push: { data: newPost } },
          { upsert: true }
        );
...
const deleteGalleryPost=(req,res)=>{
  ...
  manageSideSportsSection.findOneAndUpdate(
