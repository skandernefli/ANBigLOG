const manageBigPosts = require('../homepagemodel/manageBigPosts');
const getGalleryPost=(req,res)=>{
    manageBigPosts.findById(req.params.id)
    .then(manageBigPosts => {
      if (!manageBigPosts) {
      ...
      res.status(200).json(manageBigPosts);
    })
    .catch(error => {
      console.log(error);
      res.status(500).json({
...
const postGalleryPost= async (req,res)=>{
    try {
        ...
        const post = await manageBigPosts.findOneAndUpdate(
          {},
          { $push: { data: newPost } },
          { upsert: true }
        );
...
const deleteGalleryPost=(req,res)=>{
  ...
  manageBigPosts.findOneAndUpdate(
