const manageSportsMainSection = require('../homepagemodel/manageSportsMainSection');
const getGalleryPost=(req,res)=>{
    manageSportsMainSection.findById(req.params.id)
    .then(manageSportsMainSection => {
      if (!manageSportsMainSection) {
        return res.status(404).json({
          message: 'gallery Post not found'
        });
      }

      res.status(200).json(manageSportsMainSection);
    })
    .catch(error => {
      console.log(error);
      res.status(500).json({
...
const postGalleryPost= async (req,res)=>{
    try {
        ...
        const post = await manageSportsMainSection.findOneAndUpdate(
          {},
          { $push: { data: newPost } },
          { upsert: true }
        );
...
const deleteGalleryPost=(req,res)=>{
  ...
  manageSportsMainSection.findOneAndUpdate(
