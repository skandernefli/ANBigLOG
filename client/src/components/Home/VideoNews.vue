...
export default {
  ...
  methods: {
    async fetchsmallPostGallery() {
      const response = await fetch("http://3.145.167.18:8000/server/managevideoposts").then(res => res.json());
      const data = response[0].data;
      return this.smallPostGallery = data;
    },
    async fetchmainvideo() {
      const response = await fetch("http://3.145.167.18:8000/server/mainvideo").then(res => res.json());
